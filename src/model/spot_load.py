...
def initialize_spotload(model, SpotLoads=None):
    ...
    model.SpotLoadP_A = Param(model.Buses, initialize=SpotLoads[0]['A'].to_dict(), default=0.0, mutable=True, within=Reals)
    model.SpotLoadP_B = Param(model.Buses, initialize=SpotLoads[0]['B'].to_dict(), default=0.0, mutable=True, within=Reals)
    model.SpotLoadP_C = Param(model.Buses, initialize=SpotLoads[0]['C'].to_dict(), default=0.0, mutable=True, within=Reals)
